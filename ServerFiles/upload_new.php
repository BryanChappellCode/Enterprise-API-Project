...
if(isset($_POST['submit'])){
	...
	if(strlen($loan) != 8){
		...
		echo '</form>';
		log_error('upload-new.php', 'Loan', "Invalid Length");
	}
	...
	if(strlen($_FILES['userFile']['name']) == 0){
		...
		echo '<button type="submit" name="refresh">Go Back</button>';
		echo '</form>';
		log_error('upload-new.php', 'File', "Empty File Upload");
	}
	...
	if($_FILES['userFile']['type'] != 'application/pdf'){
		...
		echo '<button type="submit" name="refresh">Go Back</button>';
		echo '</form>';
		log_error('upload-new.php', 'File', "Invalid Extension");
	}
	...
	if($size > 10000000){
		...
		echo '<button type="submit" name="refresh">Go Back</button>';
		echo '</form>';
		log_error('upload-new.php', 'File', "File Too Large");
	}
	...
	$rst=$dblink->query($sql) or
		log_error('upload-new.php', 'SQL', $dblink->error);
	//Redirect if exists
	if($rst->num_rows != 0){
		header('Location: /upload_existing.php?loanNum='.$_POST['loanNum']);
		die();
	}
	
	//Add new Loan Num
	$sql='INSERT INTO `loan_nums` (`loan_num`) VALUES ("'.$loan.'")';
	$rst=$dblink->query($sql) or
		log_error('upload-new.php', 'SQL', $dblink->error);
	...
	if(isset($_GET['loanNum'])){
		echo '<h1 align="center" class="page-head-line">Loan Didnt Exist, So I Redirected You!</h1>';
	}
	
	echo '<div id="page-inner">';
	echo '<h1 align="center" class="page-head-line">Upload a New File to DocStorage - New Loan</h1>';
